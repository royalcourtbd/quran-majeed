...
import 'package:quran_majeed/core/external_libs/scrollable_positioned_list/src/element_registry.dart';
...
import 'package:quran_majeed/core/external_libs/scrollable_positioned_list/src/scroll_view.dart';
import 'package:quran_majeed/core/external_libs/scrollable_positioned_list/src/wrapping.dart';
...
class _PositionedListState extends State<PositionedList> {
  ...
  Widget _buildSeparatedListElement(int index) {
    if (index.isEven) {
      ...
      return widget.separatorBuilder?.call(context, index ~/ 2) ??
    ...
    if (!updateScheduled) {
      ...
      SchedulerBinding.instance.addPostFrameCallback((_) {
        ...
        for (final Element element in elements) {
          ...
          double anchor = 0;
          if (viewport is RenderViewport) anchor = viewport.anchor;
          if (viewport is CustomRenderViewport) anchor = viewport.anchor;

          final ValueKey<int>? key = element.widget.key as ValueKey<int>?;
          // Skip this element if `box` has never been laid out.
          if (box != null && !box.hasSize) continue;
          if (widget.scrollDirection == Axis.vertical && box != null) {
            ...
            if (itemOffset != null && !itemOffset.isFinite) continue;
            positions.add(
              ItemPosition(
                index: key?.value ?? -1,
                ...
                itemTrailingEdge: (widget.reverse
                            ? scrollController.position.viewportDimension -
                                (itemOffset ?? 0.1)
                            : ((itemOffset ?? 0.1) + (box?.size.width ?? 0.1)))
                        .round() /
                    scrollController.position.viewportDimension,
              ),
            );
