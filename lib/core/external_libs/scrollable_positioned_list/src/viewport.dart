...
class UnboundedRenderViewport extends RenderViewport {
    ...
    switch (axis) {
      case Axis.vertical:
        return Rect.fromLTRB(
          semanticBounds.left,
          semanticBounds.top - (_calculatedCacheExtent ?? 0),
          semanticBounds.right,
          semanticBounds.bottom + (_calculatedCacheExtent ?? 0),
        );
      default:
        return Rect.fromLTRB(
          semanticBounds.left - (_calculatedCacheExtent ?? 0),
          semanticBounds.top,
          semanticBounds.right + (_calculatedCacheExtent ?? 0),
  ...
  void performLayout() {
    ...
    final double centerOffsetAdjustment = center?.centerOffsetAdjustment ?? 0;
  ...
  double _attemptLayout(
    ...
    switch (cacheExtentStyle) {
      ...
      case CacheExtentStyle.viewport:
        _calculatedCacheExtent = mainAxisExtent * (cacheExtent ?? 0);
    }

    final double fullCacheExtent =
        mainAxisExtent + 2 * (_calculatedCacheExtent ?? 0);
    final double centerCacheOffset =
        centerOffset + (_calculatedCacheExtent ?? 0);
    ...
    if (leadingNegativeChild != null) {
      ...
      final double result = layoutChildSequence(
        ...
        cacheOrigin: (mainAxisExtent - centerOffset)
            .clamp(-(_calculatedCacheExtent ?? 0), 0.0),
      );
    ...
    return layoutChildSequence(
      ...
      cacheOrigin: centerOffset.clamp(-(_calculatedCacheExtent ?? 0), 0.0),
    );
  }
